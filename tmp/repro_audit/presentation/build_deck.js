...
// 1. Title -------------------------------------------------------------------------------
{
  ...
  s.addText("INFO1185 Inteligencia Artificial · Septiembre 2026", { x: 0.6, y: 4.6, w: 4.8, h: 0.35, fontFace: BODY, fontSize: 12, color: "9AA5AD", margin: 0, isTextBox: true });
...
// 3. Data ---------------------------------------------------------------------------------
{
  ...
  text(s, `Partición estratificada por imagen, semilla ${M.Seed}. Una copia duplicada estaba en entrenamiento y la otra en validación: se eliminó para evitar fuga. Fondos simples (blanco) y complejos (follaje, platos, madera); varios frutos por imagen.`, 0.5, 4.75, 9.0, 0.7, { fontSize: 11.5, color: C.muted });
...
// 4. Methodology ------------------------------------------------------------------------
{
  ...
  const rows = [
    ...
    [cell("Métricas finales (AUC, exactitud, sens., espec.)"), cell("Test, una única vez", { bold: true, color: C.red })],
    [cell("Robustez de la comparación"), cell(`${M.NRepeats} particiones 60/20/20 repetidas`)],
  ];
...
// 8. Seven combinations ------------------------------------------------------------------
{
  ...
  text(s, `Diferencia mejor–peor (${M.WorstCombo}): ${f3(bestSeg.dev_mean - Number(M.WorstDevJ))}, frente a DE entre imágenes ≈ ${f2(bestSeg.dev_std)}: ranking débil. R separa rojo de verde; B separa objetos saturados de fondos claros. Test: J = ${M.BestTestJ}.`, 6.9, 3.35, 2.7, 1.9, { fontSize: 11 });
...
// 16. Comparison -----------------------------------------------------------------------
{
  ...
  bullets(s, [
    ...
    `Con máscaras ideales, SFS es la única con errores (${M.SfsRepRefErr}): sobreajusta la selección.`,
  ], 7.0, 1.05, 2.6, 4.2, 11.5);
  s.addNotes("Mismas condiciones: mismas particiones, mismo umbral de Youden, mismas métricas. Las particiones repetidas no son independientes entre sí (comparten imágenes).");
}
...
// 17. Integrated analysis -------------------------------------------------------------
{
  ...
  image(s, "integration.png", 0.4, 1.0, 6.2, 2.4);
  const rows = [
    [hcell("Máscara"), hcell("Análisis"), hcell("SFS"), hcell("PCA")],
    [cell("Referencia (ideal)"), cell(M.AnRepRefErr), cell(M.SfsRepRefErr), cell(M.PcaRepRefErr)],
    [cell("K-Means final"), cell(M.AnRepKmErr), cell(M.SfsRepKmErr), cell(M.PcaRepKmErr)],
    [cell("K-Means, H aritmético"), cell(M.AnRepLinErr), cell(M.SfsRepLinErr), cell(M.PcaRepLinErr)],
  ];
  s.addTable(rows, { ...tableOpts(0.5, 3.6, 6.0, [2.4, 1.2, 1.2, 1.2]), rowH: 0.3 });
  text(s, `Errores acumulados en ${M.NRepeats} particiones (${M.NRepPredictions} predicciones por celda).`, 0.5, 4.9, 6.0, 0.35, { fontSize: 10.5, color: C.muted });
  stat(s, 6.9, 1.05, 2.7, `ρ = ${M.RhoShift}`, "Jaccard vs desplazamiento de descriptores");
  bullets(s, [
    "Peor máscara → descriptores más contaminados.",
    "Con máscara ideal, análisis y PCA no fallan: sus errores se deben a la segmentación.",
    "Tolera J bajos porque el tono medio no depende del brillo.",
    "Sin media circular, PCA pierde exactitud.",
  ], 6.9, 2.3, 2.7, 3.0, 11.5);
  s.addNotes("Respuesta a la pregunta 9. La ablación del tono muestra que la representación de H importa tanto como la selección.");
}
...
// 18. Answers -------------------------------------------------------------------------------
{
  ...
  const qa = [
    ...
    ["9. Segmentación", "Explica los errores; ρ = " + M.RhoShift + " con los descriptores."],
  ];
