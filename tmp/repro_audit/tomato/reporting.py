...
SOURCE_ES = {"reference": "Referencia (ideal)", "kmeans_raw": "K-Means sin post-proc.", "kmeans": "K-Means final",
             "kmeans_linear_hue": r"K-Means final, $\bar H$ aritmético"}
...
def write_tables(out: Path, ctx: dict):
    ...
    for _, r in seg.iterrows():
        ...
        rows.append(f"{r['combination']} & {f(r['train_mean'])} & {f(r['val_mean'])} & {dev} & {f(r['dev_post_mean'])} & "
                    ...
                    f"{r['iter_mean']:.1f} / {int(r['iter_max'])} \\\\")
    _table(tex / "segmentation.tex", "lrrrrrrr",
           r"Canales & Entr. & Valid. & Desarrollo & +Post & Oráculo & Test & Iter. (media/máx)", rows, r"\footnotesize")
    ...
    _table(tex / "mask_effect.tex", "lrrr",
           "Máscara usada para extraer $\\mathbf{x}$ & " + " & ".join(r["name"] for r in ctx["results"]), rows,
           r"\footnotesize")

    rep = ctx["repeated_summary"]
    rows = []
    for _, r in rep.iterrows():
        rows.append(f"{r['strategy']} & {src_es[r['mask_source']]} & {f(r['auc_mean'])} $\\pm$ {f(r['auc_std'])} & "
                    f"{f(r['accuracy_mean'])} $\\pm$ {f(r['accuracy_std'])} & {f(r['sensitivity_mean'])} & "
                    f"{f(r['specificity_mean'])} \\\\")
    _table(tex / "repeated.tex", "llrrrr",
           r"Estrategia & Máscara & AUC test & Exactitud test & Sens. & Espec.", rows, r"\footnotesize")
...
def write_macros(out: Path, ctx: dict):
    ...
    for key, name in (("An", "Bayes + análisis"), ("Sfs", "Bayes + SFS"), ("Pca", "PCA + Bayes")):
        ...
        rep = ctx["repeated_summary"]
        for src, S in (("kmeans", "Km"), ("reference", "Ref"), ("kmeans_linear_hue", "Lin")):
            ...
            raw = ctx["repeated"][(ctx["repeated"].strategy == name) & (ctx["repeated"].mask_source == src)]
            macros[f"{key}Rep{S}Err"] = int(raw["fp"].sum() + raw["fn"].sum())
