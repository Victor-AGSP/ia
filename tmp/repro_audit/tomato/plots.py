...
def repeated_splits(rep: pd.DataFrame, out: Path):
    ...
    sources = [("reference", "máscara de referencia"), ("kmeans", "máscara K-Means"),
               ("kmeans_linear_hue", "K-Means + H aritmético")]
    shades = {"reference": 0.35, "kmeans": 0.9, "kmeans_linear_hue": 0.6}
    hatches = {"reference": "", "kmeans": "", "kmeans_linear_hue": "//"}
    ...
    for k, (src, label) in enumerate(sources):
        ...
        axes[0].bar(x + (k - 1) * w, mean, w, yerr=sd, capsize=3, color=colors, alpha=shades[src],
                    hatch=hatches[src], edgecolor="black", linewidth=0.5, label=label)
        bars = axes[1].bar(x + (k - 1) * w, errors, w, color=colors, alpha=shades[src], hatch=hatches[src],
