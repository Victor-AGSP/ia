...
def evaluate_strategy(name, features, fit_transform, X, y, train_idx, val_idx, test_idx,
                      with_ci: bool = False) -> dict:
    """Fit on train, choose the Youden threshold on validation, evaluate once on test.
    ...
    test_scores = log_likelihood_ratio(clf, transform(X[test_idx]))
