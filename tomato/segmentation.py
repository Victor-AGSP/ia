...
def kmeans_labels(image: np.ndarray, combination: str, seed: int = config.SEED):
    ...
    # scikit-learn's OpenMP Lloyd iterations accumulate float32 sums per thread, so the
    # result can change in the last decimals with the number of cores. One thread makes
    # the segmentation identical on any machine.
    with threadpool_limits(limits=1, user_api="openmp"):
        km = KMeans(random_state=seed, **config.KMEANS_PARAMS).fit(data[sample])
        labels = km.predict(data)
    return labels.reshape(image.shape[:2]), int(km.n_iter_)
